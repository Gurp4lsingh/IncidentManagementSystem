/*
  ...
  Displays full information about a single incident.
  Allows status updates, archiving, and resetting archived incidents.
 */
...
/*
  Status transition rules (must match backend configuration)
...
/*
  ...
  Purpose: Shows detailed view of a single incident with action buttons
  Features:
  - Display all incident fields (title, description, category, severity, status, date)
  - Update status dropdown (for allowed transitions)
  - Archive button (for OPEN and RESOLVED incidents)
  - Reset button (for ARCHIVED incidents)
  
  Workflow:
  1. Load incident details on mount
  2. Display incident information
  3. Show appropriate action buttons based on current status
  4. Handle status updates, archiving, and resetting
  5. Refresh incident data after any change
 */
export default function IncidentDetails() {
  ...
  /*
    Loads incident data from API
    Sets up the allowed next status based on current status
   */
  async function load() {
    if (!id) return
    try {
  ...
  /*
    Calculates allowed next statuses based on current status
  ...
  /*
    Handles status update via workflow transition
    Updates incident status using the selected next status
   */
  ...
  /*
    Handles archiving an incident
    Only available for OPEN and RESOLVED incidents
   */
  ...
  /*
   Handles resetting an archived incident to OPEN
   Only available for ARCHIVED incidents
   */
  ...
  /*
    Determines if the current incident can be archived
    Can archive if status is OPEN or RESOLVED
   */
  const canArchive =
    item && (item.status === "OPEN" || item.status === "RESOLVED")

  /*
    Determines if the current incident can be reset
    Can reset if status is ARCHIVED
   */
