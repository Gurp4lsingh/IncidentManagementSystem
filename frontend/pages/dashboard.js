/*
  ...
  Displays KPIs and incident columns organized by status.
  Provides toggle to show/hide archived incidents.
 */
...
/*
  ...
  Purpose: Main dashboard view showing incident statistics and status columns
  Features:
  - KPI cards showing total, open, investigating, resolved, and high severity counts
  - Three status columns: OPEN, INVESTIGATING, RESOLVED
  - Checkbox to show/hide archived incidents
  - Auto-refresh when archive filter changes
 
  Data Flow:
  1. Component mounts → fetches incidents from API
  2. User toggles "Show Archived" → re-fetches with includeArchived flag
  3. Incidents are filtered by status for each column
  4. Stats are calculated from filtered incident list
 */
